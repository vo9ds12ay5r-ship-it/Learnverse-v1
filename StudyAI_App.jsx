...
import {
  ...
  Wifi,Copy,Crown,Lock,AlertCircle,
} from "lucide-react";
...
const saveProfile=p=>ls.set("edu_profile",p);
...
function SplashScreen({onContinue}){
  ...
  const BTNS=[
    {label:"Continue with Google",bg:T.white,color:T.g800,border:T.g200,icon:()=><svg width="20" height="20" viewBox="0 0 24 24"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/></svg>},
    {label:"Continue with Apple",bg:T.g800,color:T.white,border:T.g800,icon:()=><Apple size={20} color={T.white}/>},
    {label:"Continue with Phone",bg:T.white,color:T.g800,border:T.g200,icon:()=><Phone size={20}/>},
    {label:"Continue with Email",bg:T.white,color:T.g800,border:T.g200,icon:()=><Mail size={20}/>},
  ];
  return(
    <div style={{minHeight:"100vh",background:`linear-gradient(160deg,${T.p950} 0%,${T.p700} 55%,${T.p600} 100%)`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"space-between",padding:"48px 24px 40px",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:20,flex:1,justifyContent:"center"}}>
        ...
        <div style={{textAlign:"center",maxWidth:320}}>
          <h1 style={{color:T.white,fontSize:32,fontWeight:800,lineHeight:1.15,margin:"0 0 12px",letterSpacing:-.5}}>Your AI-Powered<br/><span style={{background:`linear-gradient(90deg,${T.gold},#FDE68A)`,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>Study Partner</span></h1>
          <p style={{color:"rgba(255,255,255,.65)",fontSize:15,lineHeight:1.6,margin:0}}>Crack IIT-JEE · NEET · Board Exams<br/>with personalised AI guidance.</p>
        </div>
        <div style={{display:"flex",gap:24}}>
          {[["2M+","Students"],["98%","Success"],["50K+","Questions"]].map(([n,l])=>(
            <div key={l} style={{textAlign:"center"}}><div style={{color:T.gold,fontSize:18,fontWeight:800}}>{n}</div><div style={{color:"rgba(255,255,255,.55)",fontSize:11,fontWeight:600}}>{l}</div></div>
          ))}
        </div>
      </div>
      <div style={{width:"100%",maxWidth:440,opacity:show?1:0,transform:show?"translateY(0)":"translateY(30px)",transition:"all .5s cubic-bezier(.34,1.56,.64,1)"}}>
        <div style={{background:"rgba(255,255,255,.08)",backdropFilter:"blur(20px)",borderRadius:24,border:"1.5px solid rgba(255,255,255,.15)",padding:"28px 24px 24px"}}>
          <p style={{color:"rgba(255,255,255,.7)",textAlign:"center",fontSize:13,fontWeight:600,marginBottom:18,letterSpacing:.5}}>Sign in to continue</p>
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            {BTNS.map(({label,bg,color,border,icon:Icon})=>(
              <button key={label} onClick={onContinue} style={{width:"100%",display:"flex",alignItems:"center",gap:12,padding:"14px 20px",borderRadius:14,border:`1.5px solid ${border}`,background:bg,color,fontSize:15,fontWeight:600,cursor:"pointer",boxShadow:"0 2px 8px rgba(0,0,0,.07)"}}>
                <Icon/><span style={{flex:1,textAlign:"center"}}>{label}</span>
              </button>
            ))}
          </div>
          <p style={{textAlign:"center",color:"rgba(255,255,255,.35)",fontSize:11,marginTop:20,lineHeight:1.6}}>By continuing, you agree to our <span style={{color:"rgba(255,255,255,.65)",textDecoration:"underline",cursor:"pointer"}}>Terms</span> & <span style={{color:"rgba(255,255,255,.65)",textDecoration:"underline",cursor:"pointer"}}>Privacy Policy</span></p>
        </div>
      </div>
    </div>
  );
...
function App(){
  ...
  useEffect(()=>{
    const saved=ls.get("edu_profile");
    if(saved?.name&&saved?.track){setProfile(saved);setStage("app")}
    else setStage("auth");
  },[]);
  ...
  const handleLogout=()=>{ls.del("edu_profile");setProfile(null);setStage("auth");setTab("home");go("home-dashboard")};
  ...
  if(stage==="auth")return(<><GlobalStyles/><SplashScreen onContinue={()=>setStage("onboard")}/></>);
  if(stage==="onboard")return(<><GlobalStyles/><ProfileSetupScreen onComplete={p=>{const full={...p,streak:1};saveProfile(full);setProfile(full);setStage("app")}}/></>);
